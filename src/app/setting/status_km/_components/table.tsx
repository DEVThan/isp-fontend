...
import {
  getStatusKms,
  KM_PAGE_SIZE,
} from "@/app/setting/status_km/_components/api"
import { DeleteModal } from "@/app/setting/status_km/_components/delete_modal"
import { FormModal } from "@/app/setting/status_km/_components/form_modal"
import {
  isStatusKmActive,
  KM_ACTIVE,
  KM_INACTIVE,
...
import { TablePagination } from "@/app/setting/status_km/_components/pagination"
...
export function Tables() {
  ...
  const [removing, setRemoving] = React.useState<StatusKm | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(KM_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getStatusKms({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
            ...
            <TableBody>
              {visible.map((km, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{km.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{km.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    ...
                    >
                      {isStatusKmActive(km) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
