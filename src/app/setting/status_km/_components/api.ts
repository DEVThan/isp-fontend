...
/**
 * api.ts — เส้น API ของหน้าจัดการสถานะ KM (ตาราง status_km)
 *
...
export const KM_PER_PAGE_MAX = 100

/** จำนวนแถวต่อหน้าที่หน้านี้ใช้ตอนเปิดครั้งแรก */
export const KM_PAGE_SIZE = 30
...
export type StatusKmQuery = {
  /** ค้นจาก status_km.name ฝั่งเซิร์ฟเวอร์ (ilike) — ไม่ส่ง = ไม่กรอง */
  name?: string
...
/**
 * POST /api/web/status-km-get-list — สถานะ KM ทั้งหมด (รวมที่ปิดอยู่)
 *
 * ค้นหา/กรองสถานะ/แบ่งหน้า ทำที่ฝั่งเซิร์ฟเวอร์ทั้งหมด ตารางแค่ส่งเงื่อนไขไปแล้วแสดงผลที่ได้
 * ตารางว่าง API ตอบ 200 พร้อม statuskms = [] — ไม่ใช่ error
 */
export async function getStatusKms(
  query: StatusKmQuery = {}
): Promise<StatusKmList> {
  const result = await post<StatusKmList>("status-km-get-list", {
    name: query.name ?? "",
    ...
    page: query.page ?? 1,
    per_page: query.perPage ?? KM_PAGE_SIZE,
  })
  return {
    statuskms: result?.statuskms ?? [],
...
/**
 * POST /api/web/status-km-get-option — ตัวเลือกสถานะ KM ที่ active (id + name)
 * ไม่รับพารามิเตอร์ ไม่แบ่งหน้า
 */
export async function getStatusKmOptions(): Promise<StatusKmOption[]> {
  return (
    (await post<StatusKmOption[]>("status-km-get-option", {})) ??
    []
  )
}

/**
 * POST /api/web/status-km-action — เพิ่ม/แก้ไข เส้นเดียวจบ แยกด้วย action ใน body
 *
 ...
 */
export async function saveStatusKm(
  action: StatusKmFormMode,
  values: StatusKmFormValues,
  kmId?: number
): Promise<StatusKm> {
  return (await post<StatusKm>("status-km-action", {
    action,
    id: kmId ?? 0,
    ...values,
  })) as StatusKm
...
/**
 * POST /api/web/status-km-delete — ลบสถานะ KM ตาม id (ส่งไปแค่ id เท่านั้น)
 *
 * ลบออกจากตารางจริง กู้คืนไม่ได้ · ผลลัพธ์คือแถวที่หายไป (ตารางนี้ไม่มีใครอ้างชื่อจากมัน)
 */
export async function deleteStatusKm(
  id: number
): Promise<StatusKmDeleted> {
  return (await post<StatusKmDeleted>("status-km-delete", {
    id,
  })) as StatusKmDeleted
}
