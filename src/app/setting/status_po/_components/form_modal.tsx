...
import { saveStatusPo } from "@/app/setting/status_po/_components/api"
import {
  PO_ACTIVE,
  PO_INACTIVE,
  type StatusPo,
  type StatusPoFormMode,
  type StatusPoFormValues,
...
/**
 * form_modal.tsx — ฟอร์มเพิ่ม/แก้ไขสถานะ PO ในกล่องซ้อน
 *
 * ปุ่มที่เปิดฟอร์มเป็นคนบอกโหมดมาเอง: "add" เปิดฟอร์มเปล่า "edit" เปิดพร้อมค่าของแถวนั้น
 * กดบันทึกแล้วยิง POST /api/web/status-po-action เอง เสร็จแล้วบอกพ่อผ่าน onSaved ให้โหลดตารางใหม่
 */
...
const emptyValues: StatusPoFormValues = {
  name: "",
  detail: "",
  active_status: PO_ACTIVE,
}

const toValues = (po: StatusPo | undefined): StatusPoFormValues =>
  po
    ? {
        name: po.name,
        detail: po.detail ?? "",
        active_status: po.active_status,
      }
    : emptyValues
...
export function FormModal({
  ...
  const tform = useTranslations("statuspos.form")
  const tcol = useTranslations("statuspos.columns")

  const [values, setValues] = React.useState(() => toValues(po))
   ...
   */
  const formKey = `${mode}-${po?.id ?? "new"}-${String(open)}`
  const [lastKey, setLastKey] = React.useState(formKey)
  if (formKey !== lastKey) {
    setLastKey(formKey)
    setValues(toValues(po))
    setResult(null)
    setSaving(false)
    setNameError(false)
  }

  const set = <K extends keyof StatusPoFormValues>(
    key: K,
    value: StatusPoFormValues[K]
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <form
          onSubmit={async (event) => {
            ...
            try {
              ...
              await saveStatusPo(mode, values, po?.id)
              setResult({ ok: true })
              onSaved?.()
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field id="po-status" label={tcol("status")}>
              <SelectOption
                id="po-status"
                options={statusOptions}
                value={values.active_status}
                onValueChange={(next) =>
                  set("active_status", next ?? PO_ACTIVE)
                }
          ...
          <Field id="po-detail" label={tcol("detail")}>
            <Textarea
              id="po-detail"
              rows={2}
              value={values.detail}
