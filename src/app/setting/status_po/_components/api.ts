...
/**
 * api.ts — เส้น API ของหน้าจัดการสถานะ PO (ตาราง status_po)
 *
...
export const PO_PER_PAGE_MAX = 100
...
export type StatusPoQuery = {
  /** ค้นจาก status_po.name ฝั่งเซิร์ฟเวอร์ (ilike) — ไม่ส่ง = ไม่กรอง */
  name?: string
...
/**
 * POST /api/web/status-po-get-list — สถานะ PO ทั้งหมด (รวมที่ปิดอยู่)
 *
 * ค้นหา/กรองสถานะ/แบ่งหน้า ทำที่ฝั่งเซิร์ฟเวอร์ทั้งหมด ตารางแค่ส่งเงื่อนไขไปแล้วแสดงผลที่ได้
 * ตารางว่าง API ตอบ 200 พร้อม statuspos = [] — ไม่ใช่ error
 */
export async function getStatusPos(
  query: StatusPoQuery = {}
): Promise<StatusPoList> {
  const result = await post<StatusPoList>("status-po-get-list", {
    name: query.name ?? "",
    ...
    page: query.page ?? 1,
    per_page: query.perPage ?? PO_PAGE_SIZE,
  })
  return {
    statuspos: result?.statuspos ?? [],
...
/**
 * POST /api/web/status-po-get-option — ตัวเลือกสถานะ PO ที่ active (id + name)
 * ไม่รับพารามิเตอร์ ไม่แบ่งหน้า
 */
export async function getStatusPoOptions(): Promise<StatusPoOption[]> {
  return (
    (await post<StatusPoOption[]>("status-po-get-option", {})) ??
    []
  )
}

/**
 * POST /api/web/status-po-action — เพิ่ม/แก้ไข เส้นเดียวจบ แยกด้วย action ใน body
 *
 ...
 */
export async function saveStatusPo(
  action: StatusPoFormMode,
  values: StatusPoFormValues,
  kmId?: number
): Promise<StatusPo> {
  return (await post<StatusPo>("status-po-action", {
    action,
    id: kmId ?? 0,
    ...values,
  })) as StatusPo
...
/**
 * POST /api/web/status-po-delete — ลบสถานะ PO ตาม id (ส่งไปแค่ id เท่านั้น)
 *
 * ลบออกจากตารางจริง กู้คืนไม่ได้ · ผลลัพธ์คือแถวที่หายไป (ตารางนี้ไม่มีใครอ้างชื่อจากมัน)
 */
export async function deleteStatusPo(
  id: number
): Promise<StatusPoDeleted> {
  return (await post<StatusPoDeleted>("status-po-delete", {
    id,
  })) as StatusPoDeleted
}
