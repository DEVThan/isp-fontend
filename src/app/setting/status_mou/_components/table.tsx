...
import {
  getStatusMous,
  MOU_PAGE_SIZE,
} from "@/app/setting/status_mou/_components/api"
import { DeleteModal } from "@/app/setting/status_mou/_components/delete_modal"
import { FormModal } from "@/app/setting/status_mou/_components/form_modal"
import {
  isStatusMouActive,
  MOU_ACTIVE,
  MOU_INACTIVE,
...
import { TablePagination } from "@/app/setting/status_mou/_components/pagination"
 ...
 */
const EMPTY_LIST: StatusMouList = {
  statusmous: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const [removing, setRemoving] = React.useState<StatusMou | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(MOU_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getStatusMous({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
            ...
            <TableBody>
              {visible.map((mou, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{mou.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{mou.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isStatusMouActive(mou)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isStatusMouActive(mou) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
