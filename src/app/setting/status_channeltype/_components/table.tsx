...
import {
  CHANNEL_PAGE_SIZE,
  getChannelTypes,
} from "@/app/setting/status_channeltype/_components/api"
import { DeleteModal } from "@/app/setting/status_channeltype/_components/delete_modal"
import { FormModal } from "@/app/setting/status_channeltype/_components/form_modal"
import {
  CHANNEL_ACTIVE,
  CHANNEL_INACTIVE,
  isChannelTypeActive,
  ...
  type ChannelTypeList,
} from "@/app/setting/status_channeltype/_components/model"
import { TablePagination } from "@/app/setting/status_channeltype/_components/pagination"
...
export function Tables({
  ...
  const [removing, setRemoving] = React.useState<ChannelType | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(CHANNEL_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getChannelTypes({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
  ...
  const visible = list.channeltypes
                  ...
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      ...
                      className={`border-transparent font-medium ${
                        isChannelTypeActive(channel)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isChannelTypeActive(channel) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", channel })}
