...
import { deleteChannelType } from "@/app/setting/status_channeltype/_components/api"
import type { ChannelType } from "@/app/setting/status_channeltype/_components/model"
import {
  Alert,
  AlertContent,
  AlertDescription,
...
/**
 * delete_modal.tsx — ถามยืนยันก่อนลบช่องทางการขาย แล้วค่อยยิง POST /api/web/status-channeltype-delete
 *
...
export function DeleteModal({
  ...
  const key = `${channel?.id ?? "none"}-${String(open)}`
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
          {tform("deleteDescription", { name: channel?.name ?? "" })}
        </DialogDescription>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const removed = await deleteChannelType(channel.id)
                setResult({
                  ok: true,
                  // sales_orders.channel_name เก็บเป็นชื่อ ไม่ได้ผูก id — SO ที่ค้างอยู่ไม่ได้เปลี่ยนตาม ต้องบอกให้รู้
                  message: removed.sales_orders
                    ? tform("deleteOrders", { count: removed.sales_orders })
                    : undefined,
                })
                onDeleted?.()
