...
import {
  getPaymentTypes,
  PAYMENT_TYPE_PAGE_SIZE,
} from "@/app/setting/payment_type/_components/api"
import { DeleteModal } from "@/app/setting/payment_type/_components/delete_modal"
import { FormModal } from "@/app/setting/payment_type/_components/form_modal"
import {
  isPaymentTypeActive,
  PAYMENT_TYPE_ACTIVE,
...
import { TablePagination } from "@/app/setting/payment_type/_components/pagination"
 ...
 */
const EMPTY_LIST: PaymentTypeList = {
  paymenttypes: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const [removing, setRemoving] = React.useState<PaymentType | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(PAYMENT_TYPE_PAGE_SIZE)
  ...
  const visible = list.paymenttypes
  ...
  return (
    ...
    <Card className="border-primary/10 mt-4 overflow-hidden p-0">
            ...
            <TableBody>
              {visible.map((paymentType, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{paymentType.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{paymentType.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isPaymentTypeActive(paymentType)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isPaymentTypeActive(paymentType) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", paymentType })}
                      ...
                      <Button
                        ...
                        onClick={() => setRemoving(paymentType)}
      ...
      <DeleteModal
        ...
        paymentType={removing ?? undefined}
