...
/**
 * api.ts — เส้น API ของหน้าจัดการประเภทการชำระเงิน (ตาราง payment_type)
 *
...
export const PAYMENT_TYPE_PER_PAGE_MAX = 100

/** จำนวนแถวต่อหน้าที่หน้านี้ใช้ตอนเปิดครั้งแรก */
export const PAYMENT_TYPE_PAGE_SIZE = 30
...
export type PaymentTypeQuery = {
  /** ค้นจาก payment_type.name ฝั่งเซิร์ฟเวอร์ (ilike) — ไม่ส่ง = ไม่กรอง */
  name?: string
...
/**
 * POST /api/web/payment-type-get-list — ประเภทการชำระเงินทั้งหมด (รวมที่ปิดอยู่)
 *
 * ค้นหา/กรองสถานะ/แบ่งหน้า ทำที่ฝั่งเซิร์ฟเวอร์ทั้งหมด ตารางแค่ส่งเงื่อนไขไปแล้วแสดงผลที่ได้
 * ตารางว่าง API ตอบ 200 พร้อม paymenttypes = [] — ไม่ใช่ error
 */
export async function getPaymentTypes(
  query: PaymentTypeQuery = {}
): Promise<PaymentTypeList> {
  const result = await post<PaymentTypeList>("payment-type-get-list", {
    name: query.name ?? "",
    ...
    page: query.page ?? 1,
    per_page: query.perPage ?? PAYMENT_TYPE_PAGE_SIZE,
  })
  return {
...
/**
 * POST /api/web/payment-type-get-option — ตัวเลือกประเภทการชำระเงินที่ active (id + name)
 * ไม่รับพารามิเตอร์ ไม่แบ่งหน้า
 */
export async function getPaymentTypeOptions(): Promise<PaymentTypeOption[]> {
  return (
    (await post<PaymentTypeOption[]>("payment-type-get-option", {})) ??
    []
  )
}

/**
 * POST /api/web/payment-type-action — เพิ่ม/แก้ไข เส้นเดียวจบ แยกด้วย action ใน body
 *
 ...
 */
export async function savePaymentType(
  action: PaymentTypeFormMode,
  values: PaymentTypeFormValues,
  kmId?: number
): Promise<PaymentType> {
  return (await post<PaymentType>("payment-type-action", {
    action,
    id: kmId ?? 0,
    ...values,
  })) as PaymentType
...
/**
 * POST /api/web/payment-type-delete — ลบประเภทการชำระเงินตาม id (ส่งไปแค่ id เท่านั้น)
 *
...
export async function deletePaymentType(
  id: number
): Promise<PaymentTypeDeleted> {
  return (await post<PaymentTypeDeleted>("payment-type-delete", {
    id,
  })) as PaymentTypeDeleted
}
