...
import {
  getKmProtocalls,
  PROTOCALL_PAGE_SIZE,
} from "@/app/setting/status_kmprotocall/_components/api"
import { DeleteModal } from "@/app/setting/status_kmprotocall/_components/delete_modal"
import { FormModal } from "@/app/setting/status_kmprotocall/_components/form_modal"
import {
  isKmProtocallActive,
  PROTOCALL_ACTIVE,
  PROTOCALL_INACTIVE,
  ...
  type KmProtocallList,
} from "@/app/setting/status_kmprotocall/_components/model"
import { TablePagination } from "@/app/setting/status_kmprotocall/_components/pagination"
 ...
 */
const EMPTY_LIST: KmProtocallList = {
  statuskmprotocalls: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const tpage = useTranslations("statuskmprotocalls")
  ...
  const [list, setList] = React.useState<KmProtocallList>(EMPTY_LIST)
  ...
  const [removing, setRemoving] = React.useState<KmProtocall | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(PROTOCALL_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getKmProtocalls({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
  ...
  const visible = list.statuskmprotocalls
            ...
            <TableBody>
              {visible.map((protocall, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{protocall.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{protocall.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isKmProtocallActive(protocall)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isKmProtocallActive(protocall) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", protocall })}
