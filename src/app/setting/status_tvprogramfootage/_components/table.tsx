...
import {
  getTvFootages,
  FOOTAGE_PAGE_SIZE,
} from "@/app/setting/status_tvprogramfootage/_components/api"
import { DeleteModal } from "@/app/setting/status_tvprogramfootage/_components/delete_modal"
import { FormModal } from "@/app/setting/status_tvprogramfootage/_components/form_modal"
import {
  isTvFootageActive,
  FOOTAGE_ACTIVE,
  FOOTAGE_INACTIVE,
  type TvFootage,
  type TvFootageFormMode,
  type TvFootageList,
} from "@/app/setting/status_tvprogramfootage/_components/model"
import { TablePagination } from "@/app/setting/status_tvprogramfootage/_components/pagination"
import { SelectOption } from "@/app/setting/status_tvprogramfootage/_components/selectoption"
 ...
 */
const EMPTY_LIST: TvFootageList = {
...
export function Tables() {
  ...
  const tpage = useTranslations("statustvprogramfootages")
  const tall = useTranslations("common")
  const tr = useTranslations("statustvprogramfootages")
  const tcol = useTranslations("statustvprogramfootages.columns")

  const [list, setList] = React.useState<TvFootageList>(EMPTY_LIST)
  ...
  const [form, setForm] = React.useState<{
    mode: TvFootageFormMode
    footage?: TvFootage
  } | null>(null)
  /** แถวที่กำลังถามยืนยันจะลบ — null คือปิดกล่อง */
  const [removing, setRemoving] = React.useState<TvFootage | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(FOOTAGE_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getTvFootages({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
  ...
  const statusOptions = [
    { value: FOOTAGE_ACTIVE, label: tr("active") },
    { value: FOOTAGE_INACTIVE, label: tr("inactive") },
  ]

  // ตารางไม่กรองเองแล้ว แถวที่ได้มาคือหน้าที่ API ตัดมาให้ตรงเงื่อนไขอยู่แล้ว
  const visible = list.statustvprogramfootages
            ...
            <TableBody>
              {visible.map((footage, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{footage.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{footage.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isTvFootageActive(footage)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isTvFootageActive(footage) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", footage })}
