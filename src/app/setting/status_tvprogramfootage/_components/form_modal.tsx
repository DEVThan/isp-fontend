...
import { saveTvFootage } from "@/app/setting/status_tvprogramfootage/_components/api"
import {
  FOOTAGE_ACTIVE,
  FOOTAGE_INACTIVE,
  type TvFootage,
  type TvFootageFormMode,
  type TvFootageFormValues,
...
/**
 * form_modal.tsx — ฟอร์มเพิ่ม/แก้ไขสถานะ TV Program Footage ในกล่องซ้อน
 *
 * ปุ่มที่เปิดฟอร์มเป็นคนบอกโหมดมาเอง: "add" เปิดฟอร์มเปล่า "edit" เปิดพร้อมค่าของแถวนั้น
 * กดบันทึกแล้วยิง POST /api/web/status-tvprogramfootage-action เอง เสร็จแล้วบอกพ่อผ่าน onSaved ให้โหลดตารางใหม่
 */
...
const emptyValues: TvFootageFormValues = {
  name: "",
  detail: "",
  active_status: FOOTAGE_ACTIVE,
}

const toValues = (footage: TvFootage | undefined): TvFootageFormValues =>
  footage
    ? {
        name: footage.name,
        detail: footage.detail ?? "",
        active_status: footage.active_status,
      }
    : emptyValues
...
export function FormModal({
  ...
  const tform = useTranslations("statustvprogramfootages.form")
  const tcol = useTranslations("statustvprogramfootages.columns")

  const [values, setValues] = React.useState(() => toValues(footage))
   ...
   */
  const formKey = `${mode}-${footage?.id ?? "new"}-${String(open)}`
  const [lastKey, setLastKey] = React.useState(formKey)
  if (formKey !== lastKey) {
    setLastKey(formKey)
    setValues(toValues(footage))
    setResult(null)
    setSaving(false)
    setNameError(false)
  }

  const set = <K extends keyof TvFootageFormValues>(
    key: K,
    value: TvFootageFormValues[K]
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <form
          onSubmit={async (event) => {
            ...
            try {
              ...
              await saveTvFootage(mode, values, footage?.id)
              setResult({ ok: true })
              onSaved?.()
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field id="footage-status" label={tcol("status")}>
              <SelectOption
                id="footage-status"
                options={statusOptions}
                value={values.active_status}
                onValueChange={(next) =>
                  set("active_status", next ?? FOOTAGE_ACTIVE)
                }
          ...
          <Field id="footage-detail" label={tcol("detail")}>
            <Textarea
              id="footage-detail"
              rows={2}
              value={values.detail}
