...
import {
  getShiptmentTypes,
  SHIPTMENT_PAGE_SIZE,
} from "@/app/setting/status_shiptmenttype/_components/api"
import { DeleteModal } from "@/app/setting/status_shiptmenttype/_components/delete_modal"
import { FormModal } from "@/app/setting/status_shiptmenttype/_components/form_modal"
import {
  isShiptmentTypeActive,
  SHIPTMENT_ACTIVE,
  SHIPTMENT_INACTIVE,
  type ShiptmentType,
  type ShiptmentTypeFormMode,
  type ShiptmentTypeList,
} from "@/app/setting/status_shiptmenttype/_components/model"
import { TablePagination } from "@/app/setting/status_shiptmenttype/_components/pagination"
import { SelectOption } from "@/app/setting/status_shiptmenttype/_components/selectoption"
 ...
 */
const EMPTY_LIST: ShiptmentTypeList = {
  shiptmenttypes: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const tpage = useTranslations("shiptmenttypes")
  const tall = useTranslations("common")
  const tr = useTranslations("shiptmenttypes")
  const tcol = useTranslations("shiptmenttypes.columns")

  const [list, setList] = React.useState<ShiptmentTypeList>(EMPTY_LIST)
  ...
  const [form, setForm] = React.useState<{
    mode: ShiptmentTypeFormMode
    type?: ShiptmentType
  } | null>(null)
  /** แถวที่กำลังถามยืนยันจะลบ — null คือปิดกล่อง */
  const [removing, setRemoving] = React.useState<ShiptmentType | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(SHIPTMENT_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getShiptmentTypes({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
  ...
  const statusOptions = [
    { value: SHIPTMENT_ACTIVE, label: tr("active") },
    { value: SHIPTMENT_INACTIVE, label: tr("inactive") },
  ]

  // ตารางไม่กรองเองแล้ว แถวที่ได้มาคือหน้าที่ API ตัดมาให้ตรงเงื่อนไขอยู่แล้ว
  const visible = list.shiptmenttypes
            ...
            <TableBody>
              {visible.map((type, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{type.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{type.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isShiptmentTypeActive(type)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isShiptmentTypeActive(type) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", type })}
