...
import { saveShiptmentType } from "@/app/setting/status_shiptmenttype/_components/api"
import {
  SHIPTMENT_ACTIVE,
  SHIPTMENT_INACTIVE,
  type ShiptmentType,
  type ShiptmentTypeFormMode,
  type ShiptmentTypeFormValues,
...
/**
 * form_modal.tsx — ฟอร์มเพิ่ม/แก้ไขประเภทการจัดส่งในกล่องซ้อน
 *
 * ปุ่มที่เปิดฟอร์มเป็นคนบอกโหมดมาเอง: "add" เปิดฟอร์มเปล่า "edit" เปิดพร้อมค่าของแถวนั้น
 * กดบันทึกแล้วยิง POST /api/web/status-shiptmenttype-action เอง เสร็จแล้วบอกพ่อผ่าน onSaved ให้โหลดตารางใหม่
 */
...
const emptyValues: ShiptmentTypeFormValues = {
  name: "",
  detail: "",
  active_status: SHIPTMENT_ACTIVE,
}

const toValues = (type: ShiptmentType | undefined): ShiptmentTypeFormValues =>
  type
    ? {
        name: type.name,
        detail: type.detail ?? "",
...
export function FormModal({
  ...
  const [values, setValues] = React.useState(() => toValues(type))
   ...
   */
  const formKey = `${mode}-${type?.id ?? "new"}-${String(open)}`
  const [lastKey, setLastKey] = React.useState(formKey)
  if (formKey !== lastKey) {
    setLastKey(formKey)
    setValues(toValues(type))
    setResult(null)
    setSaving(false)
    setNameError(false)
  }

  const set = <K extends keyof ShiptmentTypeFormValues>(
    key: K,
    value: ShiptmentTypeFormValues[K]
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <form
          onSubmit={async (event) => {
            ...
            try {
              ...
              await saveShiptmentType(mode, values, type?.id)
              setResult({ ok: true })
              onSaved?.()
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field id="shiptment-status" label={tcol("status")}>
              <SelectOption
                id="shiptment-status"
                options={statusOptions}
                value={values.active_status}
                onValueChange={(next) =>
                  set("active_status", next ?? SHIPTMENT_ACTIVE)
                }
          ...
          <Field id="shiptment-detail" label={tcol("detail")}>
            <Textarea
              id="shiptment-detail"
              rows={2}
              value={values.detail}
