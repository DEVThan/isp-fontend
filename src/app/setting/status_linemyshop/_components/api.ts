...
import type {
  ...
  LineMyShopDeleted,
  LineMyShopFormMode,
  LineMyShopFormValues,
  LineMyShopList,
  LineMyShopOption,
} from "@/app/setting/status_linemyshop/_components/model"

/**
 * api.ts — เส้น API ของหน้าจัดการสถานะ Line My Shop (ตาราง status_line_my_shop)
 *
...
export const SHOP_PER_PAGE_MAX = 100
...
export type LineMyShopQuery = {
  /** ค้นจาก status_line_my_shop.name ฝั่งเซิร์ฟเวอร์ (ilike) — ไม่ส่ง = ไม่กรอง */
  name?: string
...
/**
 * POST /api/web/status-linemyshop-get-list — สถานะ Line My Shop ทั้งหมด (รวมที่ปิดอยู่)
 *
 * ค้นหา/กรองสถานะ/แบ่งหน้า ทำที่ฝั่งเซิร์ฟเวอร์ทั้งหมด ตารางแค่ส่งเงื่อนไขไปแล้วแสดงผลที่ได้
 * ตารางว่าง API ตอบ 200 พร้อม statuslinemyshops = [] — ไม่ใช่ error
 */
export async function getLineMyShops(
  query: LineMyShopQuery = {}
): Promise<LineMyShopList> {
  const result = await post<LineMyShopList>("status-linemyshop-get-list", {
    name: query.name ?? "",
    ...
    page: query.page ?? 1,
    per_page: query.perPage ?? SHOP_PAGE_SIZE,
  })
  return {
    statuslinemyshops: result?.statuslinemyshops ?? [],
...
/**
 * POST /api/web/status-linemyshop-get-option — ตัวเลือกสถานะ Line My Shop ที่ active (id + name)
 * ไม่รับพารามิเตอร์ ไม่แบ่งหน้า
 */
export async function getLineMyShopOptions(): Promise<LineMyShopOption[]> {
  return (
    (await post<LineMyShopOption[]>("status-linemyshop-get-option", {})) ??
    []
  )
}

/**
 * POST /api/web/status-linemyshop-action — เพิ่ม/แก้ไข เส้นเดียวจบ แยกด้วย action ใน body
 *
 ...
 */
export async function saveLineMyShop(
  action: LineMyShopFormMode,
  values: LineMyShopFormValues,
  kmId?: number
): Promise<LineMyShop> {
  return (await post<LineMyShop>("status-linemyshop-action", {
    action,
    id: kmId ?? 0,
    ...values,
  })) as LineMyShop
...
/**
 * POST /api/web/status-linemyshop-delete — ลบสถานะ Line My Shop ตาม id (ส่งไปแค่ id เท่านั้น)
 *
 * ลบออกจากตารางจริง กู้คืนไม่ได้ · ผลลัพธ์คือแถวที่หายไป (ตารางนี้ไม่มีใครอ้างชื่อจากมัน)
 */
export async function deleteLineMyShop(
  id: number
): Promise<LineMyShopDeleted> {
  return (await post<LineMyShopDeleted>("status-linemyshop-delete", {
    id,
  })) as LineMyShopDeleted
}
