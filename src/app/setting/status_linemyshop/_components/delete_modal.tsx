...
import { deleteLineMyShop } from "@/app/setting/status_linemyshop/_components/api"
import type { LineMyShop } from "@/app/setting/status_linemyshop/_components/model"
import {
  Alert,
  AlertContent,
  AlertDescription,
...
/**
 * delete_modal.tsx — ถามยืนยันก่อนลบสถานะ Line My Shop แล้วค่อยยิง POST /api/web/status-linemyshop-delete
 *
...
export function DeleteModal({
  ...
  const key = `${shop?.id ?? "none"}-${String(open)}`
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
          {tform("deleteDescription", { name: shop?.name ?? "" })}
        </DialogDescription>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                await deleteLineMyShop(shop.id)
                setResult({ ok: true })
                onDeleted?.()
