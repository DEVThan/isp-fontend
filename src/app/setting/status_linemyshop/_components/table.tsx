...
import {
  getLineMyShops,
  SHOP_PAGE_SIZE,
} from "@/app/setting/status_linemyshop/_components/api"
import { DeleteModal } from "@/app/setting/status_linemyshop/_components/delete_modal"
import { FormModal } from "@/app/setting/status_linemyshop/_components/form_modal"
import {
  isLineMyShopActive,
  SHOP_ACTIVE,
  SHOP_INACTIVE,
...
import { TablePagination } from "@/app/setting/status_linemyshop/_components/pagination"
 ...
 */
const EMPTY_LIST: LineMyShopList = {
  statuslinemyshops: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const tpage = useTranslations("statuslinemyshops")
  ...
  const [list, setList] = React.useState<LineMyShopList>(EMPTY_LIST)
  ...
  const [removing, setRemoving] = React.useState<LineMyShop | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(SHOP_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getLineMyShops({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
  ...
  const visible = list.statuslinemyshops
            ...
            <TableBody>
              {visible.map((shop, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{shop.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{shop.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isLineMyShopActive(shop)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isLineMyShopActive(shop) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", shop })}
