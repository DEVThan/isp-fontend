...
import type {
  StatusPayment,
  StatusPaymentDeleted,
  StatusPaymentFormMode,
  StatusPaymentFormValues,
  StatusPaymentList,
  StatusPaymentOption,
} from "@/app/setting/status_payment/_components/model"

/**
 * api.ts — เส้น API ของหน้าจัดการสถานะการชำระเงิน (ตาราง status_payment)
 *
...
export const PAYMENT_PER_PAGE_MAX = 100
...
export type StatusPaymentQuery = {
  /** ค้นจาก status_payment.name ฝั่งเซิร์ฟเวอร์ (ilike) — ไม่ส่ง = ไม่กรอง */
  name?: string
...
/**
 * POST /api/web/status-payment-get-list — สถานะการชำระเงินทั้งหมด (รวมที่ปิดอยู่)
 *
 * ค้นหา/กรองสถานะ/แบ่งหน้า ทำที่ฝั่งเซิร์ฟเวอร์ทั้งหมด ตารางแค่ส่งเงื่อนไขไปแล้วแสดงผลที่ได้
 * ตารางว่าง API ตอบ 200 พร้อม statuspayments = [] — ไม่ใช่ error
 */
export async function getStatusPayments(
  query: StatusPaymentQuery = {}
): Promise<StatusPaymentList> {
  const result = await post<StatusPaymentList>("status-payment-get-list", {
    name: query.name ?? "",
    ...
    page: query.page ?? 1,
    per_page: query.perPage ?? PAYMENT_PAGE_SIZE,
  })
  return {
    statuspayments: result?.statuspayments ?? [],
...
/**
 * POST /api/web/status-payment-get-option — ตัวเลือกสถานะการชำระเงินที่ active (id + name)
 * ไม่รับพารามิเตอร์ ไม่แบ่งหน้า
 */
export async function getStatusPaymentOptions(): Promise<StatusPaymentOption[]> {
  return (
    (await post<StatusPaymentOption[]>("status-payment-get-option", {})) ??
    []
  )
}

/**
 * POST /api/web/status-payment-action — เพิ่ม/แก้ไข เส้นเดียวจบ แยกด้วย action ใน body
 *
 ...
 */
export async function saveStatusPayment(
  action: StatusPaymentFormMode,
  values: StatusPaymentFormValues,
  kmId?: number
): Promise<StatusPayment> {
  return (await post<StatusPayment>("status-payment-action", {
    action,
    id: kmId ?? 0,
    ...values,
  })) as StatusPayment
...
/**
 * POST /api/web/status-payment-delete — ลบสถานะการชำระเงินตาม id (ส่งไปแค่ id เท่านั้น)
 *
 * ลบออกจากตารางจริง กู้คืนไม่ได้ · ผลลัพธ์คือแถวที่หายไป (ตารางนี้ไม่มีใครอ้างชื่อจากมัน)
 */
export async function deleteStatusPayment(
  id: number
): Promise<StatusPaymentDeleted> {
  return (await post<StatusPaymentDeleted>("status-payment-delete", {
    id,
  })) as StatusPaymentDeleted
}
