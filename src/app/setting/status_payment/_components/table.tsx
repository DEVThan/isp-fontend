...
import {
  getStatusPayments,
  PAYMENT_PAGE_SIZE,
} from "@/app/setting/status_payment/_components/api"
import { DeleteModal } from "@/app/setting/status_payment/_components/delete_modal"
import { FormModal } from "@/app/setting/status_payment/_components/form_modal"
import {
  isStatusPaymentActive,
  PAYMENT_ACTIVE,
  PAYMENT_INACTIVE,
  type StatusPayment,
  type StatusPaymentFormMode,
  type StatusPaymentList,
} from "@/app/setting/status_payment/_components/model"
import { TablePagination } from "@/app/setting/status_payment/_components/pagination"
import { SelectOption } from "@/app/setting/status_payment/_components/selectoption"
 ...
 */
const EMPTY_LIST: StatusPaymentList = {
  statuspayments: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const tpage = useTranslations("statuspayments")
  const tall = useTranslations("common")
  const tr = useTranslations("statuspayments")
  const tcol = useTranslations("statuspayments.columns")

  const [list, setList] = React.useState<StatusPaymentList>(EMPTY_LIST)
  ...
  const [form, setForm] = React.useState<{
    mode: StatusPaymentFormMode
    payment?: StatusPayment
  } | null>(null)
  /** แถวที่กำลังถามยืนยันจะลบ — null คือปิดกล่อง */
  const [removing, setRemoving] = React.useState<StatusPayment | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(PAYMENT_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getStatusPayments({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
  ...
  const statusOptions = [
    { value: PAYMENT_ACTIVE, label: tr("active") },
    { value: PAYMENT_INACTIVE, label: tr("inactive") },
  ]

  // ตารางไม่กรองเองแล้ว แถวที่ได้มาคือหน้าที่ API ตัดมาให้ตรงเงื่อนไขอยู่แล้ว
  const visible = list.statuspayments
            ...
            <TableBody>
              {visible.map((payment, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{payment.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{payment.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isStatusPaymentActive(payment)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isStatusPaymentActive(payment) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", payment })}
