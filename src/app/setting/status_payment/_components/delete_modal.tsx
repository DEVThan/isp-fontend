...
import { deleteStatusPayment } from "@/app/setting/status_payment/_components/api"
import type { StatusPayment } from "@/app/setting/status_payment/_components/model"
import {
  Alert,
  AlertContent,
  AlertDescription,
...
/**
 * delete_modal.tsx — ถามยืนยันก่อนลบสถานะการชำระเงินแล้วค่อยยิง POST /api/web/status-payment-delete
 *
...
export function DeleteModal({
  ...
  const key = `${payment?.id ?? "none"}-${String(open)}`
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
          {tform("deleteDescription", { name: payment?.name ?? "" })}
        </DialogDescription>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                await deleteStatusPayment(payment.id)
                setResult({ ok: true })
                onDeleted?.()
