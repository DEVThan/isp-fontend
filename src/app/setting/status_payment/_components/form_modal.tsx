...
import { saveStatusPayment } from "@/app/setting/status_payment/_components/api"
import {
  PAYMENT_ACTIVE,
  PAYMENT_INACTIVE,
  type StatusPayment,
  type StatusPaymentFormMode,
  type StatusPaymentFormValues,
...
/**
 * form_modal.tsx — ฟอร์มเพิ่ม/แก้ไขสถานะการชำระเงินในกล่องซ้อน
 *
 * ปุ่มที่เปิดฟอร์มเป็นคนบอกโหมดมาเอง: "add" เปิดฟอร์มเปล่า "edit" เปิดพร้อมค่าของแถวนั้น
 * กดบันทึกแล้วยิง POST /api/web/status-payment-action เอง เสร็จแล้วบอกพ่อผ่าน onSaved ให้โหลดตารางใหม่
 */
...
const emptyValues: StatusPaymentFormValues = {
  name: "",
  detail: "",
  active_status: PAYMENT_ACTIVE,
}

const toValues = (payment: StatusPayment | undefined): StatusPaymentFormValues =>
  payment
    ? {
        name: payment.name,
        detail: payment.detail ?? "",
        active_status: payment.active_status,
      }
    : emptyValues
...
export function FormModal({
  ...
  const tform = useTranslations("statuspayments.form")
  const tcol = useTranslations("statuspayments.columns")

  const [values, setValues] = React.useState(() => toValues(payment))
   ...
   */
  const formKey = `${mode}-${payment?.id ?? "new"}-${String(open)}`
  const [lastKey, setLastKey] = React.useState(formKey)
  if (formKey !== lastKey) {
    setLastKey(formKey)
    setValues(toValues(payment))
    setResult(null)
    setSaving(false)
    setNameError(false)
  }

  const set = <K extends keyof StatusPaymentFormValues>(
    key: K,
    value: StatusPaymentFormValues[K]
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <form
          onSubmit={async (event) => {
            ...
            try {
              ...
              await saveStatusPayment(mode, values, payment?.id)
              setResult({ ok: true })
              onSaved?.()
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field id="payment-status" label={tcol("status")}>
              <SelectOption
                id="payment-status"
                options={statusOptions}
                value={values.active_status}
                onValueChange={(next) =>
                  set("active_status", next ?? PAYMENT_ACTIVE)
                }
          ...
          <Field id="payment-detail" label={tcol("detail")}>
            <Textarea
              id="payment-detail"
              rows={2}
              value={values.detail}
