...
import { deleteMenu } from "@/app/setting/menu/_components/api"
import type { Menu } from "@/app/setting/menu/_components/model"
import {
  Alert,
  AlertContent,
  AlertDescription,
...
/**
 * delete_modal.tsx — ถามยืนยันก่อนลบเมนู แล้วค่อยยิง POST /api/web/menu-delete
 *
...
export function DeleteModal({
  ...
  const key = `${menu?.id ?? "none"}-${String(open)}`
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
          {tform("deleteDescription", { name: menu?.name ?? "" })}
        </DialogDescription>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                const removed = await deleteMenu(menu.id)
                setResult({
                  ok: true,
                  // ลูกของแถวที่ลบไปจะกลายเป็นเมนูลอย ต้องบอกให้รู้
                  message: removed.children
                    ? tform("deleteOrphans", { count: removed.children })
                    : undefined,
                })
                onDeleted?.()
