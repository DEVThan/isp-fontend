...
export function SelectOption({
  ...
  label?: string
  /**
   * ทำเครื่องหมายว่าช่องนี้ยังไม่ได้เลือกทั้งที่จำเป็น
   *
   * ไม่ได้ใช้ required ของ Base UI เพราะ input ที่มันซ่อนไว้ให้ฟอร์มตรวจ เป็นกล่อง 1x1 px
   * ที่มุมบนซ้ายของกล่องซ้อน ป้าย "Please fill out this field." ของเบราว์เซอร์จึงไปโผล่ที่นั่น
   * ไม่ได้ชี้ที่ช่องนี้ — ผู้เรียกตรวจเองแล้วส่ง invalid มาทำเครื่องหมายกับข้อความใต้ช่องแทน
   */
  invalid?: boolean
}) {
    ...
    >
      <div className="relative">
        <Combobox.Trigger
          ...
          // ใช้คลาสสลับเอง ไม่ใช้ variant aria-invalid: เพราะ Tailwind v4 ห่อ variant ด้วย :where()
          // ความจำเพาะจึงเท่ากับ border-input แล้วแพ้ลำดับใน stylesheet (ทดสอบแล้วขอบไม่เปลี่ยนสีจริง)
          className={`${
            invalid
