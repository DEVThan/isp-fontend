...
import type {
  ImportantDoc,
  ImportantDocDeleted,
  ImportantDocFormMode,
  ImportantDocFormValues,
  ImportantDocList,
  ImportantDocOption,
} from "@/app/setting/status_importantdoc/_components/model"

/**
 * api.ts — เส้น API ของหน้าจัดการเอกสารสำคัญ (ตาราง important_doc)
 *
...
export const DOC_PER_PAGE_MAX = 100

/** จำนวนแถวต่อหน้าที่หน้านี้ใช้ตอนเปิดครั้งแรก */
export const DOC_PAGE_SIZE = 30
...
export type ImportantDocQuery = {
  /** ค้นจาก important_doc.name ฝั่งเซิร์ฟเวอร์ (ilike) — ไม่ส่ง = ไม่กรอง */
  name?: string
...
/**
 * POST /api/web/status-importantdoc-get-list — เอกสารสำคัญทั้งหมด (รวมที่ปิดอยู่)
 *
 * ค้นหา/กรองสถานะ/แบ่งหน้า ทำที่ฝั่งเซิร์ฟเวอร์ทั้งหมด ตารางแค่ส่งเงื่อนไขไปแล้วแสดงผลที่ได้
 * ตารางว่าง API ตอบ 200 พร้อม importantdocs = [] — ไม่ใช่ error
 */
export async function getImportantDocs(
  query: ImportantDocQuery = {}
): Promise<ImportantDocList> {
  const result = await post<ImportantDocList>("status-importantdoc-get-list", {
    name: query.name ?? "",
    ...
    page: query.page ?? 1,
    per_page: query.perPage ?? DOC_PAGE_SIZE,
  })
  return {
    importantdocs: result?.importantdocs ?? [],
...
/**
 * POST /api/web/status-importantdoc-get-option — ตัวเลือกเอกสารสำคัญที่ active (id + name)
 * ไม่รับพารามิเตอร์ ไม่แบ่งหน้า
 */
export async function getImportantDocOptions(): Promise<ImportantDocOption[]> {
  return (
    (await post<ImportantDocOption[]>("status-importantdoc-get-option", {})) ??
    []
  )
}

/**
 * POST /api/web/status-importantdoc-action — เพิ่ม/แก้ไข เส้นเดียวจบ แยกด้วย action ใน body
 *
 ...
 */
export async function saveImportantDoc(
  action: ImportantDocFormMode,
  values: ImportantDocFormValues,
  kmId?: number
): Promise<ImportantDoc> {
  return (await post<ImportantDoc>("status-importantdoc-action", {
    action,
    id: kmId ?? 0,
    ...values,
  })) as ImportantDoc
...
/**
 * POST /api/web/status-importantdoc-delete — ลบเอกสารสำคัญตาม id (ส่งไปแค่ id เท่านั้น)
 *
 * ลบออกจากตารางจริง กู้คืนไม่ได้ · ผลลัพธ์คือแถวที่หายไป (ตารางนี้ไม่มีใครอ้างชื่อจากมัน)
 */
export async function deleteImportantDoc(
  id: number
): Promise<ImportantDocDeleted> {
  return (await post<ImportantDocDeleted>("status-importantdoc-delete", {
    id,
  })) as ImportantDocDeleted
}
