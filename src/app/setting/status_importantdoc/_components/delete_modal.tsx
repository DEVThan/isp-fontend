...
import { deleteImportantDoc } from "@/app/setting/status_importantdoc/_components/api"
import type { ImportantDoc } from "@/app/setting/status_importantdoc/_components/model"
import {
  Alert,
  AlertContent,
  AlertDescription,
...
/**
 * delete_modal.tsx — ถามยืนยันก่อนลบเอกสารสำคัญแล้วค่อยยิง POST /api/web/status-importantdoc-delete
 *
...
export function DeleteModal({
  ...
  const key = `${doc?.id ?? "none"}-${String(open)}`
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
          {tform("deleteDescription", { name: doc?.name ?? "" })}
        </DialogDescription>
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                await deleteImportantDoc(doc.id)
                setResult({ ok: true })
                onDeleted?.()
