...
import { saveImportantDoc } from "@/app/setting/status_importantdoc/_components/api"
import {
  DOC_ACTIVE,
  DOC_INACTIVE,
  type ImportantDoc,
  type ImportantDocFormMode,
  type ImportantDocFormValues,
...
/**
 * form_modal.tsx — ฟอร์มเพิ่ม/แก้ไขเอกสารสำคัญในกล่องซ้อน
 *
 * ปุ่มที่เปิดฟอร์มเป็นคนบอกโหมดมาเอง: "add" เปิดฟอร์มเปล่า "edit" เปิดพร้อมค่าของแถวนั้น
 * กดบันทึกแล้วยิง POST /api/web/status-importantdoc-action เอง เสร็จแล้วบอกพ่อผ่าน onSaved ให้โหลดตารางใหม่
 */
...
const emptyValues: ImportantDocFormValues = {
  name: "",
  detail: "",
  active_status: DOC_ACTIVE,
}

const toValues = (doc: ImportantDoc | undefined): ImportantDocFormValues =>
  doc
    ? {
        name: doc.name,
        detail: doc.detail ?? "",
...
export function FormModal({
  ...
  const tform = useTranslations("importantdocs.form")
  const tcol = useTranslations("importantdocs.columns")

  const [values, setValues] = React.useState(() => toValues(doc))
   ...
   */
  const formKey = `${mode}-${doc?.id ?? "new"}-${String(open)}`
  const [lastKey, setLastKey] = React.useState(formKey)
  if (formKey !== lastKey) {
    setLastKey(formKey)
    setValues(toValues(doc))
    setResult(null)
    setSaving(false)
    setNameError(false)
  }

  const set = <K extends keyof ImportantDocFormValues>(
    key: K,
    value: ImportantDocFormValues[K]
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <form
          onSubmit={async (event) => {
            ...
            try {
              ...
              await saveImportantDoc(mode, values, doc?.id)
              setResult({ ok: true })
              onSaved?.()
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field id="doc-status" label={tcol("status")}>
              <SelectOption
                id="doc-status"
                options={statusOptions}
                value={values.active_status}
                onValueChange={(next) =>
                  set("active_status", next ?? DOC_ACTIVE)
                }
          ...
          <Field id="doc-detail" label={tcol("detail")}>
            <Textarea
              id="doc-detail"
              rows={2}
              value={values.detail}
