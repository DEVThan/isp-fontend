...
import {
  getImportantDocs,
  DOC_PAGE_SIZE,
} from "@/app/setting/status_importantdoc/_components/api"
import { DeleteModal } from "@/app/setting/status_importantdoc/_components/delete_modal"
import { FormModal } from "@/app/setting/status_importantdoc/_components/form_modal"
import {
  isImportantDocActive,
  DOC_ACTIVE,
  DOC_INACTIVE,
  type ImportantDoc,
  type ImportantDocFormMode,
  type ImportantDocList,
} from "@/app/setting/status_importantdoc/_components/model"
import { TablePagination } from "@/app/setting/status_importantdoc/_components/pagination"
import { SelectOption } from "@/app/setting/status_importantdoc/_components/selectoption"
 ...
 */
const EMPTY_LIST: ImportantDocList = {
  importantdocs: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const tpage = useTranslations("importantdocs")
  const tall = useTranslations("common")
  const tr = useTranslations("importantdocs")
  const tcol = useTranslations("importantdocs.columns")

  const [list, setList] = React.useState<ImportantDocList>(EMPTY_LIST)
  ...
  const [form, setForm] = React.useState<{
    mode: ImportantDocFormMode
    doc?: ImportantDoc
  } | null>(null)
  /** แถวที่กำลังถามยืนยันจะลบ — null คือปิดกล่อง */
  const [removing, setRemoving] = React.useState<ImportantDoc | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(DOC_PAGE_SIZE)
  ...
  const load = (
    ...
    timer.current = setTimeout(async () => {
      try {
        const result = await getImportantDocs({
          name: next.name.trim(),
          status: next.status,
          page: next.page,
  ...
  const statusOptions = [
    { value: DOC_ACTIVE, label: tr("active") },
    { value: DOC_INACTIVE, label: tr("inactive") },
  ]

  // ตารางไม่กรองเองแล้ว แถวที่ได้มาคือหน้าที่ API ตัดมาให้ตรงเงื่อนไขอยู่แล้ว
  const visible = list.importantdocs
            ...
            <TableBody>
              {visible.map((doc, index) => (
                ...
                >
                  ...
                  <TableCell className="pt-1 pb-1 font-medium">{doc.name}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate">{doc.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                      className={`border-transparent font-medium ${
                        isImportantDocActive(doc)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isImportantDocActive(doc) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
