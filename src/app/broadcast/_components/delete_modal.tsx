...
import { deleteBroadcast } from "@/app/broadcast/_components/api"
import type { Broadcast } from "@/app/broadcast/_components/model"
import {
  Alert,
  AlertContent,
  AlertDescription,
...
/**
 * delete_modal.tsx — ถามยืนยันก่อนลบช่องทางแล้วค่อยยิง POST /api/web/broadcast-delete
 *
...
export function DeleteModal({
  ...
  const key = `${broadcast?.id ?? "none"}-${String(open)}`
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
          {tform("deleteDescription", { name: broadcast?.name ?? "" })}
        </DialogDescription>
        ...
        <DialogFooter className="from-primary/12 border-border/60 bg-transparent bg-gradient-to-r via-transparent to-transparent">
          ...
          <Button
            variant="destructive"
            disabled={deleting || !broadcast}
            onClick={async () => {
              if (!broadcast || deleting) return
              setDeleting(true)
              setResult(null)
              try {
                // ส่งไปแค่ id ตามที่เส้นนี้ต้องการ · ไม่มีตารางไหนอ้างแถวนี้ จึงไม่มีอะไรต้องเตือนต่อ
