...
import {
  getBroadcasts,
  BROADCAST_PAGE_SIZE,
} from "@/app/broadcast/_components/api"
import { DeleteModal } from "@/app/broadcast/_components/delete_modal"
import { FormModal } from "@/app/broadcast/_components/form_modal"
...
import { TablePagination } from "@/app/broadcast/_components/pagination"
...
/** ตัวกรองของตาราง — name ตรงกับพารามิเตอร์ของ broadcast-get-list · status null = ไม่กรอง */
type Filters = { name: string; status: string | null }

const EMPTY_FILTERS: Filters = { name: "", status: null }
 ...
 */
const EMPTY_LIST: BroadcastList = {
  broadcasts: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const [list, setList] = React.useState<BroadcastList>(EMPTY_LIST)
  ...
  const [removing, setRemoving] = React.useState<Broadcast | null>(null)
  ...
  const [pageSize, setPageSize] = React.useState(BROADCAST_PAGE_SIZE)
            ...
            <TableBody>
              {visible.map((broadcast, index) => (
                ...
                >
                  ...
                  {/* โลโก้ + ชื่อในช่องเดียว — โลโก้เปิดไม่ขึ้นก็เหลือแค่ชื่อ */}
                  <TableCell className="pt-1 pb-1">
                    <div className="flex items-center gap-2">
                      <BroadcastLogo src={broadcast.logo} alt={broadcast.name} version={logoVersion} className="size-7" />
                      <span className="font-medium">{broadcast.name}</span>
                    </div>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground font-mono text-xs">{broadcast.tel}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[220px] truncate" title={broadcast.email}>{broadcast.email}</TableCell>
                  <TableCell className="pt-1 pb-1 text-muted-foreground max-w-[260px] truncate" title={broadcast.detail ?? undefined}>{broadcast.detail}</TableCell>
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      variant="secondary"
                    ...
                    >
                      {isBroadcastActive(broadcast) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
                  <TableCell className="pt-1 pb-1 pr-6 text-right">
                    <div className="flex justify-end gap-0.5">
                      <Button
                        ...
                        size="icon"
                        aria-label={t("edit")}
                        onClick={() => setForm({ mode: "edit", broadcast })}
