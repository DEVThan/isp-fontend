...
import { deleteProductItem } from "@/app/product/product_item/_components/api"
import type { ProductItem } from "@/app/product/product_item/_components/model"
import {
  Alert,
  AlertContent,
  AlertDescription,
...
/**
 * delete_modal.tsx — ถามยืนยันก่อนลบสินค้าแล้วค่อยยิง POST /api/web/product-item-delete
 *
 ...
 *
 * ตารางนี้ไม่เหมือนทะเบียนอื่น — order_items.product_id เป็น foreign key ชี้มาที่ products จริง ๆ
 * สินค้าที่มีบรรทัด SO ใช้อยู่ลบไม่ได้ API นับให้ก่อนแล้วตอบ 400 พร้อมจำนวนบรรทัด (ไม่ได้ลบอะไรเลย)
 * ตอนนี้สินค้าเกือบทุกตัวถูก SO ใช้อยู่ จึงเตือนไว้ตั้งแต่ก่อนกด ไม่ใช่รอให้ API ปฏิเสธ
 */
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <DialogDescription>
          {tform("deleteDescription", { name: item?.product_name ?? "" })}
        </DialogDescription>

        {/* เตือนก่อนกด — สินค้าที่ขายไปแล้วลบไม่ได้ */}
        <Alert>
          <TriangleAlert />
          <AlertContent>
            <AlertDescription>{tform("deleteBlocked")}</AlertDescription>
          </AlertContent>
        </Alert>
