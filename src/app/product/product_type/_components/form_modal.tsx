...
import { saveProductType } from "@/app/product/product_type/_components/api"
import {
  TYPE_ACTIVE,
  TYPE_INACTIVE,
  type ProductType,
...
/**
 * form_modal.tsx — ฟอร์มเพิ่ม/แก้ไขประเภทสินค้าในกล่องซ้อน
 *
 * ปุ่มที่เปิดฟอร์มเป็นคนบอกโหมดมาเอง: "add" เปิดฟอร์มเปล่า "edit" เปิดพร้อมค่าของแถวนั้น
 * กดบันทึกแล้วยิง POST /api/web/product-type-action เอง เสร็จแล้วบอกพ่อผ่าน onSaved ให้โหลดตารางใหม่
 */
...
const emptyValues: ProductTypeFormValues = {
  name: "",
  detail: "",
  active_status: TYPE_ACTIVE,
}

const toValues = (type: ProductType | undefined): ProductTypeFormValues =>
  type
    ? {
        name: type.name,
        detail: type.detail ?? "",
...
export function FormModal({
  ...
  const tform = useTranslations("producttypes.form")
  const tcol = useTranslations("producttypes.columns")

  const [values, setValues] = React.useState(() => toValues(type))
  const [saving, setSaving] = React.useState(false)
  /** ชื่อประเภทสินค้ายังว่างตอนกดบันทึก — ตรวจเองแทน required ของเบราว์เซอร์ */
  ...
  const set = <K extends keyof ProductTypeFormValues>(
    key: K,
    value: ProductTypeFormValues[K]
      ...
      <DialogContent className="sm:max-w-xl">
        ...
        <form
          onSubmit={async (event) => {
            ...
            try {
              ...
              await saveProductType(mode, values, type?.id)
              setResult({ ok: true })
              onSaved?.()
        ...
        >
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field id="type-status" label={tcol("status")}>
              <SelectOption
                id="type-status"
                options={statusOptions}
                value={values.active_status}
                onValueChange={(next) =>
                  set("active_status", next ?? TYPE_ACTIVE)
                }
          ...
          <Field id="type-detail" label={tcol("detail")}>
            <Textarea
              id="type-detail"
              rows={2}
              value={values.detail}
