...
import {
  getProductTypes,
  TYPE_PAGE_SIZE,
} from "@/app/product/product_type/_components/api"
import { DeleteModal } from "@/app/product/product_type/_components/delete_modal"
import { FormModal } from "@/app/product/product_type/_components/form_modal"
import {
  isProductTypeActive,
  TYPE_ACTIVE,
  TYPE_INACTIVE,
  type ProductType,
  type ProductTypeFormMode,
  type ProductTypeList,
} from "@/app/product/product_type/_components/model"
import { TablePagination } from "@/app/product/product_type/_components/pagination"
import { SelectOption } from "@/app/product/product_type/_components/selectoption"
 ...
 */
const EMPTY_LIST: ProductTypeList = {
  producttypes: [],
  total: 0,
  page: 1,
  per_page: 0,
...
export function Tables() {
  ...
  const tpage = useTranslations("producttypes")
  const tall = useTranslations("common")
  const tr = useTranslations("producttypes")
  const tcol = useTranslations("producttypes.columns")

  const [list, setList] = React.useState<ProductTypeList>(EMPTY_LIST)
  ...
  const [form, setForm] = React.useState<{
    mode: ProductTypeFormMode
    type?: ProductType
  } | null>(null)
  /** แถวที่กำลังถามยืนยันจะลบ — null คือปิดกล่อง */
  const [removing, setRemoving] = React.useState<ProductType | null>(null)

  const [nameQuery, setNameQuery] = React.useState("")
  /** null = ไม่กรองสถานะ (ทั้งหมด) */
  const [status, setStatus] = React.useState<string | null>(null)
  const [page, setPage] = React.useState(1)
  const [pageSize, setPageSize] = React.useState(TYPE_PAGE_SIZE)
  ...
  const statusOptions = [
    { value: TYPE_ACTIVE, label: tr("active") },
    { value: TYPE_INACTIVE, label: tr("inactive") },
  ]

  // ตารางไม่กรองเองแล้ว แถวที่ได้มาคือหน้าที่ API ตัดมาให้ตรงเงื่อนไขอยู่แล้ว
  const visible = list.producttypes
                  ...
                  <TableCell className="pt-1 pb-1">
                    <Badge
                      ...
                      className={`border-transparent font-medium ${
                        isProductTypeActive(type)
                          ? "bg-success/12 text-success-ink hover:bg-success/12"
                          : "bg-muted text-muted-foreground hover:bg-muted"
                      }`}
                    >
                      {isProductTypeActive(type) ? tr("active") : tr("inactive")}
                    </Badge>
                  </TableCell>
